...
def detrend(
    ...
    **kwargs
    ...
    """
    High-level function to detrend signal
    ...
    data : pd.DataFrame or dict
        raw data to detrend, if None, timestamps, signal and isosbestic must be provided as array-like
    ...
    by : str or list
        ...
        the data
        divides the data into groups and detrends each group separately, typically you should pass enough
        columns to uniquely identify each session
    steps : bool
        if True, returns intermediate steps as a pd.DataFrame
    method : str
        method to use for detrending, one of [airPLS, biexponential decay] or None if only artifact removal
        is needed
    smooth : int or None
        length of the smoothing moving average window; used to smooth the signal and isosbestic before detrending
    standardize: bool
        if True, calculates z-scores from the dFF after detrending and artifact removal
    kwargs : dict
    ...
    Returns
    -------
    pd.DataFrame or pd.Series with the detrendd signal

    """

    detrender = _Detrender(
        data=data,
...
class _Detrender:
    ...
    def __call__(self):
        ...
        # if no isosbestic is provided, only detrend, no artifact removal
        if self.isosbestic is None:
            if self.method == "airPLS":
                method = self.detrend_with_airPLS
            elif self.method == "biexponential decay":
                method = self.detrend_with_biexponential_decay
        # if isosbestic is provided, detrend and remove artifact
        else:
            if self.method == "airPLS":
                method = self.detrend_with_airPLS_and_remove_artifact
            elif self.method == "biexponential decay":
                method = self.detrend_with_biexponential_decay_and_remove_artifact
        ...
        def z_score(method, data):
            # perform detrending and artifact removal first
            data = method(data)
        ...
        r = self.data.groupby(self.by, sort=False, as_index=False).apply(method)
    ...
    def detrend_with_airPLS_and_remove_artifact(self, data):
        """
        Detrend signal using airPLS and remove artifact
        ...
        result = self.detrend_with_airPLS(data)
        signal = result["dFF"] if self.steps else result

        # pass detrendd signal to remove artifact on this one, not on the original signal
        result = self.remove_artifact(data, signal)
        return result

    def detrend_with_biexponential_decay_and_remove_artifact(self, data):
        """
        Detrend signal using biexponential decay and remove artifacts using isosbestic
        ...
        result = self.detrend_with_biexponential_decay(data)
        signal = result["dFF"] if self.steps else result
    ...
    def detrend_with_airPLS(self, data):
        """
        Low-level function to detrend signal using airPLS; estimates a baseline and calculates dFF
        Parameters
        ----------
        data
        ...
        Returns
        -------
        pd.Series with detrendd signal or df with intermediate steps as additional columns
    ...
    def detrend_with_biexponential_decay(self, data):
        """
        Low-level function to detrend signal using biexponential decay
    ...
    def airPLS(x, lambda_=1e4, porder=1, itermax=50):
        ...
        for i in range(1, itermax + 1):
            z = _Detrender.whittaker_smooth(x, w, lambda_, porder)
            d = x - z
